# Main file
# Combines all the functions together according to the desired
...
def main():
    ...
    if user == "1":
        username, login = signin(s3)
    elif user == "2" :
        username, login = createuser(s3)

    print("Loading your data ...")
    getstuff(username, s3)

    # Defined key bindings for user friendly operation
    upload_key = "ctrl+u"
    download_key = "ctrl+d"
    refresh = "ctrl+r"
    close = "ctrl+c"
    ...
    # Assiging functionalities to corresponding key bindings.
    keyboard.add_hotkey(upload_key, lambda : upload(username, s3))
    keyboard.add_hotkey(download_key, lambda : download(username, s3))
...
# Function to print key bindings and their uses.
def gethelp():
...
# Function to print Frequently Asked Questions and their answers.
def askFAQ():
    print(print_colored("\n----------------------------------- HiSt : FAQ -------------------------------------\n", 'yellow'), "")
